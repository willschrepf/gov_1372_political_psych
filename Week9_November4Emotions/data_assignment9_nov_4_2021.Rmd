...
Study1_processing1 <- Study1_preprocess %>% 
  rename_with(.cols = c(Q13, Q14, Q15), ~c("PUT COMMA SEPARATED LIST OF VARIABLE NAMES AS STRINGS HERE FOR THESE THREE VARIABLES IN ORDER"))
```
...
Study1_processing2 <- Study1_processing1 %>% 
  rename_with(.cols = last_col(offset = 5):last_col(), ~c("PUT COMMA SEPARATED LIST OF VARIABLE NAMES AS STRINGS HERE FOR THE LAST SIX VARIABLES IN ORDER"))
```
...
Study1_processing3 <- Study1_processing2 %>% 
  rename_with(.cols = contains("Q11"), ~c("PUT COMMA SEPARATED LIST OF EMOTIONS NAMES AS STRINGS HERE FOR THESE VARIABLES IN ORDER"))
```
...
Study1_processing5 <- Study1_processing4 %>% 
  mutate(across(.cols = "PUT TARGET COLUMNS HERE", ~na_if(.,"PUT TARGET VALUE HERE")))
```
