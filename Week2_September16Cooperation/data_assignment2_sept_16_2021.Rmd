...
```{r q1_rearrange, echo = FALSE, message=FALSE}
...
best_by_score <- player_data_long %>%
  group_by(player) %>%
  summarize(total_score = sum(score)) %>%
  arrange(-total_score)

best_by_wins <- pd_data %>%
  select(winner) %>%
  mutate(punisher_wins = sum(pd_data$winner == "punisher")) %>%
  mutate(retaliater_wins = sum(pd_data$winner == "retaliater")) %>%
  mutate(backstabber_wins = sum(pd_data$winner == "backstabber")) %>%
  mutate(grudger_wins = sum(pd_data$winner == "grudger")) %>%
  mutate(grumpy_wins = sum(pd_data$winner == "grumpy")) %>%
  mutate(prober_wins = sum(pd_data$winner == "prober")) %>%
  select(-winner) %>%
  pivot_longer(c(punisher_wins, retaliater_wins, backstabber_wins, grudger_wins, grumpy_wins, prober_wins)) %>%
  head(6) %>%
  arrange(-value)
...
```{r make_plot, echo = FALSE, message = FALSE}
p <- player_data_long %>%
  ggplot(aes(y = player, x = opponent, fill = score)) +
  geom_tile() +
   scale_fill_gradient(low = "lightgreen",
                      high = "darkgreen",
                      guide = "colorbar") +
  guides(colour=FALSE) + labs(title = "Success of Strategies Against One Another in Axelrod Simulations", subtitle = "Score of PLAYER (y-axis) versus a given OPPONENT (on the x-axis)", y = "Player", x = "Opponent", fill = "Score of PLAYER\nversus OPPONENT") + 
