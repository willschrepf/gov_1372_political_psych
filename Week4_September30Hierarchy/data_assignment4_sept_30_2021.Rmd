...
sdo_improved <- sdo %>%
  ...
  mutate(rep_vote = case_when(
    presvote16 == "Trump" ~ 1,
    presvote16 == "Clinton" ~ 0,
    presvote16 == "Stein" ~ 0,
    presvote16 == "Johnson" ~ 0
  ))
```

```{r q6_part2}
model_fit <- lm(rep_vote ~ sdo5 + female + white + educ + age + pid3 + ideo5, data=sdo_improved)

plot_summs(model_fit, scale = TRUE, inner_ci_level = .9) +
  labs(title = "Multiple Regression Estimates of Republican Voters")
summ(model_fit, model.info = FALSE, model.fit = FALSE)
```
