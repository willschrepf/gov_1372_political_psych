...
```{r}
model <- lm(init ~ suffrage + polity + polity*suffrage, s_data)
summary(model)
```
...
suffrage_0 <- with(s_data, data.frame(polity = 1:10, suffrage = 0))

preds_0 <- predict(model, suffrage_0, type="response", se.fit=TRUE)
...
suffrage_1 <- with(s_data, data.frame(polity = 1:10, suffrage = 1))

preds_1 <- predict(model, suffrage_1, type="response", se.fit=TRUE)

predf_1 <- preds_1$fit
lower_1 <- preds_1$fit - (1.96*preds_1$se.fit) # lower bounds
upper_1 <- preds_1$fit + (1.96*preds_1$se.fit) # upper bounds

overall <- data.frame(polity=as.double(names(predf_1)), no_suffrage_pred=predf_0,
                      lower_ns_pred = lower_0, upper_ns_pred = upper_0, 
                      suffrage_pred=predf_1, 
                      lower_s_pred = lower_1, upper_s_pred = upper_1, 
                      row.names=NULL)


overall %>% ggplot(aes(x = polity)) +
  geom_line(aes(y = no_suffrage_pred, color = "No Suffrage")) +
  geom_line(aes(y = lower_ns_pred, color = "No Suffrage"), linetype = "dotted") +
  geom_line(aes(y = upper_ns_pred, color = "No Suffrage"), linetype = "dotted") +
  geom_line(aes(y = suffrage_pred, color = "Suffrage")) +
  geom_line(aes(y = lower_s_pred, color = "Suffrage"), linetype = "dotted") +
  geom_line(aes(y = upper_s_pred, color = "Suffrage"), linetype = "dotted") +
  labs(x = "Polity", y = "Predicted Conflict", title = "Predicted Conflict for Suffrage and Non-Suffragre Dyads Across Polity Scores", subtitle = "With 95% Confidence Intervals")
